    ...
    *,
    ...
    with env.begin(write=False) as txn:
        for index in range(length):
            ...
            key = f"mask_{str(index).zfill(zfill)}".encode("utf-8")
            mask_bytes = txn.get(key)
            ...
            buffer = BytesIO(mask_bytes)
            mask = pickle.load(buffer)
    ...
    data_mask = th.stack(data_mask,0)
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with self.env.begin(write=False) as txn:
            ...
            key = f"mask_{str(index).zfill(self.zfill)}".encode("utf-8")
            mask_bytes = txn.get(key)
